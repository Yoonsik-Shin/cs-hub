...
- [x] Gradle Wrapper 추가
- [x] 통합 검증 명령 제공
- [x] 공개 저장소 민감정보 및 생성물 점검

완료 조건: 새로운 환경에서 문서에 적힌 명령으로 동일한 검증을 실행할 수 있다.

### Milestone 2. 백엔드 대표 코드

- [x] 문의 통합과 이메일 스레드 회귀 테스트 보강
- [x] 시간과 시스템 작업자 하드코딩 제거
- [x] 입력 검증과 이메일 스레드 규칙 분리
- [x] 업무 오류를 명시적인 타입으로 표현
- [x] PII 암호화 및 인증 경계 테스트 추가

완료 조건: 대표 업무 규칙이 클래스 이름과 테스트 이름만으로 읽히며 모든 테스트가 통과한다.

### Milestone 3. 프론트엔드 대표 코드

- [x] 목록 조회, 페이지 캐시, 자동 갱신 로직 분리
- [x] 일괄 선택 정책을 순수 함수로 분리
- [x] 선택한 상세 문의 유지 로직 테스트
- [x] 이미지 뷰어와 상세 타임라인 분리
- [x] 기존 lint 오류 제거
...
- [x] 핵심 설계 결정 ADR 작성
- [x] 대표 시나리오 code tour 작성
- [x] README를 문제 → 설계 판단 → 코드 → 테스트 흐름으로 재작성
- [x] 아키텍처 문서와 현재 코드 일치 여부 검증

완료 조건: README에서 5분 안에 대표 코드와 그 테스트까지 이동할 수 있다.

### Milestone 5. 실행 화면과 최종 검증

- [x] 전체 Docker Compose 환경 실행
- [x] 대표 업무 흐름 수동 검증
- [x] 데스크톱 및 좁은 화면 스크린샷 촬영
- [x] README 이미지 최적화 및 대체 텍스트 작성
- [x] 깨진 링크, Mermaid, 실행 명령 최종 확인

완료 조건: 새 checkout에서 실행과 검증을 재현할 수 있고 README의 화면이 실제 현재 구현과 일치한다.

### Milestone 6. 화면 오케스트레이터 후속 분리

- [x] 계정 메뉴와 네이버 세션 위젯을 `App.tsx`에서 분리
- [x] 사이드바 폭·접힘·위젯 순서·저장 필터 순서를 `AdminSidebar`로 이동
- [x] 상세 작업 이력·회신 조회와 외부 갱신을 `useInquiryActivity`로 이동
- [x] 필드 변경 감지·사유 검증·이미지 업로드·저장을 `useInquiryFieldEditor`로 이동
- [x] 채널 메타데이터·디바이스 섹션과 확인 모달을 표현 컴포넌트로 분리
- [x] 데스크톱, 상세 편집, 확장 사이드바, `390x844` 화면 회귀 검증

결과적으로 직전 기준 `App.tsx` 2,343줄은 1,349줄, `InquiryDetailPanel.tsx` 2,240줄은 1,418줄이 되었다. 파일 길이만 줄이기 위해 조각내지 않고 사이드바 개인화, 상세 활동 조회, 필드 편집 트랜잭션, 채널별 표현처럼 독립적으로 변경되는 책임을 기준으로 분리했다.
