...
### Milestone 7. 횡단 정책 일관성

- [x] HTTP 요청과 백엔드 오류 변환을 공통 클라이언트로 통합
- [x] 상태·채널·날짜·사용자 코드·이미지 정책을 단일 모듈로 통합
- [x] 생성·상세 편집의 이미지 검증과 압축을 동일하게 적용
- [x] 모달 생명주기와 toast·inline·confirm 피드백 기준 통합
- [x] 답변 등록과 상태 변경을 한 서버 트랜잭션으로 처리
- [x] 한 상세 저장의 상태·필드 변경을 한 서버 트랜잭션으로 처리
- [x] 외부 파일 삭제를 DB 커밋 이후로 이동
- [x] 관리자 DB와 `.htpasswd` 변경에 원자적 파일 교체와 롤백 보상 적용
- [x] 공유 경계를 우회하는 구현을 소스 스캔 테스트로 차단

현재 기준 `App.tsx`는 1,213줄, `InquiryDetailPanel.tsx`는 1,393줄이다. 이번 단계는 줄 수보다 같은 사용자 의도에 같은 검증·오류·트랜잭션 정책이 적용되는지를 완료 조건으로 삼았다.

### Milestone 8. 화면 조립 경계 마무리

- [x] 목록 열·배치 선택·페이지네이션을 `InquiryWorkspacePane`으로 이동
- [x] 선택 상세와 빈 상태를 `SelectedInquiryPane`으로 이동
- [x] 최상위 운영자·네이버 세션·통계·리사이즈 상태를 전용 훅으로 이동
- [x] 상세 답변·상태·즐겨찾기 명령을 `useInquiryActions`로 이동
- [x] 상세 헤더·처리 콘솔·이력 열을 독립 컴포넌트로 이동
- [x] 데스크톱 상호작용과 `390x844` overflow 회귀 검증

현재 기준 `App.tsx`는 776줄, `InquiryDetailPanel.tsx`는 786줄이다. 두 파일은 데이터 흐름과 화면 배치를 조율하고, 독립적인 상태 생명주기와 화면 영역은 각각 훅과 컴포넌트가 소유한다.
