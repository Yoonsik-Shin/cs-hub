...
import com.ttam.cs.feature.inquiry.usecase.DeleteCustomFilterUseCase;
import com.ttam.cs.feature.inquiry.usecase.GetCustomFiltersUseCase;
import com.ttam.cs.feature.inquiry.usecase.SaveCustomFilterUseCase;
...
@RestController
@RequestMapping("/api/v1/inquiries/custom-filters")
@RequiredArgsConstructor
public class CustomFilterController {

    private final GetCustomFiltersUseCase getCustomFiltersUseCase;
    private final SaveCustomFilterUseCase saveCustomFilterUseCase;
    private final DeleteCustomFilterUseCase deleteCustomFilterUseCase;
    ...
    public ResponseEntity<List<CustomFilter>> getCustomFilters(
        ...
        return ResponseEntity.ok(getCustomFiltersUseCase.execute(remoteUser));
    }
    ...
    public ResponseEntity<CustomFilter> saveCustomFilter(
        ...
        return saveCustomFilterUseCase.execute(remoteUser, request.name(), request.filterData())
    ...
    public ResponseEntity<Void> deleteCustomFilter(
        ...
        DeleteCustomFilterUseCase.Result result = deleteCustomFilterUseCase.execute(remoteUser, id);
        if (result == DeleteCustomFilterUseCase.Result.BAD_REQUEST) {
            return ResponseEntity.badRequest().build();
        }
        if (result == DeleteCustomFilterUseCase.Result.NOT_FOUND) {
