...
PM 및 기획자와 실제 CS 이력 관리에서 겪는 문제, 우선순위와 구현 제약을 논의한 뒤 **약 4주의 실개발 기간**에 완성할 범위를 합의했습니다. 개발은 **09:00~15:00, 하루 6시간** 일정으로 진행했으며, 제한된 시간 안에서 핵심 업무 흐름을 먼저 연결하고 운영 예외와 품질을 보강했습니다.

개발 과정에서는 **OpenAI·Anthropic·Google 3사의 생성형 AI**를 요구사항 구조화, 설계 대안 탐색, 코드 구현, 리뷰와 문서화에 교차 활용했습니다. 한 모델의 답을 그대로 채택하지 않고 서로 다른 제안을 비교한 뒤, 기존 코드 구조와 합의한 업무 규칙에 맞는 방향을 직접 결정했습니다.

AI를 활용하더라도 다음 기술적 경계는 개발자가 직접 정의하고 검증했습니다.

- 폴링과 재시도에서 발생할 수 있는 중복 실행, 멱등성과 데이터 정합성
- 상태 변경·작업 기록·파일 삭제 사이의 트랜잭션 원자성과 실패 처리
- 개인정보의 저장 암호화와 검색용 HMAC 분리
- 관리자·n8n·API·브라우저 워커 사이의 인증 및 신뢰 경계
- 세션 만료, 외부 서비스 실패, 자동 갱신 중 사용자 선택 보존 같은 운영 예외

생성된 결과는 코드 리뷰와 자동 테스트, lint·build, Docker Compose 실행 및 실제 브라우저 동작으로 확인했습니다. AI를 개발 속도와 탐색 범위를 높이는 도구로 활용하되, **최종 설계 판단과 코드 품질에 대한 책임은 개발자가 갖는 방식**으로 작업했습니다.
